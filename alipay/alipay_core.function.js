...
exports.createLinkstringEncode = function(para){
    //return qs.stringify(para);
    var ls = '';
    for(var k in para){
        if(k=='biz_content'){
            ls = ls + k + '=' + encodeURIComponent(JSON.stringify(para[k])) + '&';
        }else if(k=='sign'){
            ls = ls + k + '=' +
                encodeURIComponent(para[k]) + '&';
        }else{
            ls = ls + k + '=' + encodeURIComponent(para[k]) + '&';
        }
...
exports.getHttpResponseGET = function(url,cacert_url, callback){
    ...
    request.get(parsed_url.href,null,function(a,b,c){
        if(c){
            callback(c);
        }
    })
...
exports.getHttpResponse = function(url, callback){
    ...
    //request.get(url,null,function(a,b,c){
    //    if(c){
    //        callback(c);
    //    }
    //})
