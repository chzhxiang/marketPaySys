...
AlipaySubmit.prototype.buildRequestMysign = function(para_sort){
    ...
    if(sign_type == "MD5"){
        ...
        mysign = "";
        var privatePem = fs.readFileSync(this.alipay_config['private_key']);
        var privateKey = privatePem.toString('ascii');
        console.log('加密：'+prestr);
        var singer = crypto.createSign('RSA-SHA1');
        singer.update(prestr, 'utf8');
        mysign= singer.sign(privateKey, 'base64');
        console.log(mysign);
        //mysign=encodeURIComponent(mysign);
        //console.log(encodeURIComponent(mysign));
        //console.log(encodeURI(mysign));
        //mysign=encodeURIComponent(crypto.createHmac('sha1', publicKey).update(new Buffer(prestr, 'utf8')).digest('base64'))
...
AlipaySubmit.prototype.buildRequestPara = function(para_temp){
    ...
    para_sort = core_funcs.argSort(para_sort);
    return para_sort;
}
...
AlipaySubmit.prototype.buildRequestHttp = function (para_temp, callback) {
    ...
    //远程获取数据
    core_funcs.getHttpResponseGET(this.alipay_gateway_new, this.alipay_config['cacert'],request_data,this.alipay_config['input_charset'].toLowerCase().trim(), callback);
}
 ...
 * @param callback
 */
AlipaySubmit.prototype.refund = function (para_temp, callback) {
    //待请求参数数组字符串
    var para = this.buildRequestPara(para_temp);
    var request_data = core_funcs.createLinkstringEncode(para);
    //para.biz_content=JSON.stringify(para.biz_content);
    //var request_data = qs.stringify(para);
    //qs.stringify(para)
    //request_data=encodeURIComponent(request_data);
    var url="https://mapi.alipay.com/gateway.do?"+request_data;
    console.log(url);

    //远程获取数据 //get
    core_funcs.getHttpResponse(url,callback);
    //callback('123');

    //POST 带公钥
    //core_funcs.getHttpResponsePOST(url,this.alipay_config['cacert'],request_data,this.alipay_config['input_charset'].toLowerCase().trim(), function(c){
    //    console.log(c);
    //});
};
 ...
 * @param callback
 */
AlipaySubmit.prototype.telWebPay = function (para_temp, callback) {
    //待请求参数数组字符串
    var para = this.buildRequestPara(para_temp);
    var request_data = core_funcs.createLinkstringEncode(para);
    var url="https://mapi.alipay.com/gateway.do?"+request_data;
 ...
 * @param callback
 */
AlipaySubmit.prototype.payByNative = function (para_temp, callback) {
    //待请求参数数组字符串
    var para = this.buildRequestPara(para_temp);
    var request_data = core_funcs.createLinkstringEncode(para);
    var url="	https://openapi.alipay.com/gateway.do?"+request_data;
    console.log(url);
...
AlipaySubmit.prototype.webPay = function (para_temp, callback) {
    //待请求参数数组字符串
    var para = this.buildRequestPara(para_temp);
    var request_data = core_funcs.createLinkstringEncode(para);
    var url="	https://openapi.alipay.com/gateway.do?"+request_data;
 ...
 * @param callback
 */
AlipaySubmit.prototype.getRefundUrl = function (para_temp) {
    //待请求参数数组字符串
    var para = this.buildRequestPara(para_temp);
    var request_data = core_funcs.createLinkstringEncode(para);
    var url="https://mapi.alipay.com/gateway.do?"+request_data;
